...
def get_all_targets_and_history(today_str):
    ...
    if os.path.exists(files['Stock']):
        try:
            df = pd.read_csv(files['Stock'], encoding='utf-8-sig')
            # 🔴 여기는 TARGET_SCORE 이상인 것만 가져옴 (자동 발굴)
            codes = df[df['score_total'] >= TARGET_SCORE]['code'].astype(str).str.zfill(6).tolist()
    ...
    if os.path.exists(files['History']):
        try:
            df = pd.read_csv(files['History'], encoding='utf-8-sig')

            # 🔴 점수 필터링 제거! (검색한 건 무조건 가져온다)
        ...
        for row in data_list:
            ...
            if idx:
                for col, val in row.items():
                    if col in df.columns: df.at[idx[0], col] = val
            else:
...
def run_updater():
    ...
    while True:
        try:
            ...
            # 출력: history_codes에 있는 종목 개수 표시
            print(
                f"🔥 [Cycle Start] 전체 대상: {len(targets)}개 (검색기록: {len([c for c in targets if c in history_codes])}개) | KOSPI {k_val * 100:+.2f}%")
            ...
            for code, is_etf in targets.items():
                try:
                    ...
                    if not os.path.exists(file_path):
                        ...
                        if os.path.exists(os.path.join(alt_dir, f"A{code}.csv")):
                            file_path = os.path.join(alt_dir, f"A{code}.csv")
                        else:
                            continue
