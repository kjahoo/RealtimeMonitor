...
[구조 — 2026-08-19 변경: 이벤트 구동 Claude CLI 평가 추가]
- scheduler.py 가 build_pending 직후 미평가 60점+ 를 감지하면 Claude Code CLI(claude.exe -p, 헤드리스)를 즉시 기동해 AI 평가를 수행한다(maybe_spawn_claude_eval, 락+30분 타임아웃+동일종목셋 30분 쿨다운, 로그 logs/{날짜}_claude_cli_eval.log). CLI 세션도 이 CLAUDE.md 지침을 그대로 따른다.
- 따라서 Cowork 예약작업(stock-60-ai-eval-day)은 CLI 미로그인/실패 대비 백업 폴링이다. 실행 시 미평가 종목이 없으면(대부분) 한 줄 보고 후 즉시 종료가 정상.
- CLI 전제조건: claude /login 1회 필요(미로그인 시 rc=1로 실패하고 백업 폴링이 대신 처리).
...
(리서치 효율화 — Report_AI 참고) 평가 리서치 전, C:\Projects\RealtimeMonitor\Report_AI_mirror 폴더(원본 OneDrive\Documents\Report_AI 를 Windows 작업스케줄러 "ReportAI_Mirror" 가 매일 07:50 robocopy /MIR 로 동기화 — 2026-08-19 설정. 원본 경로는 예약 세션에서 접근 불가하므로 미러만 사용)에서 **최신일자 하위폴더**를 찾아, 이번에 평가할 종목과 **중복되는 종목의 기존 분석 보고서가 있으면 참고자료로 먼저 읽는다**(장마감 후 score 0.6+ 종목 보고서가 날짜별로 저장됨). 중복 종목은 해당 보고서 내용을 토대로 하되 web_search 로 최신 실적·뉴스만 보강해 작성하면 효율적이다. 폴더/파일이 없거나 중복 종목이 없으면 무시하고 평소대로 web_search 로 새로 평가한다. (이 폴더는 참고 전용 — 쓰기·수정 금지, 결과는 여전히 logs 폴더에만 저장.)
...
- **(주가변수 배제 — 2026-08-19 추가) 시세 등 주가관련 변수(등락률·고점/저점 대비 낙폭·52주 신고가/신저가·거래량·수급·차트/기술적 지표·시장분위기·변동성)는 별도 툴(V3 모델·집행 게이트·execution_monitor)이 전담하므로 claude_score 에 일절 반영 금지 — 감점("추격 회피")·가점("낙폭 과대 매력") 양방향 모두 금지. 기준가로 PER/PBR 계산은 허용하되 배수 수준 자체만 근거로 사용.**
- **(일관성 앵커 — 2026-08-19 추가) 평가 전 logs\ 에서 해당 종목의 가장 최근 {날짜}_claude_results(_all).json 을 찾아 직전 claude_score 를 앵커로 삼는다. 직전 평가 이후 새로운 중대 뉴스·공시가 없으면 직전 점수 ±3 이내 유지(기공시 정보 재해석으로 점수 변경 금지). ±4 이상 변경 또는 밴드 변경 시 verdict 에 "직전 N점 → 이번 M점, 변경 사유(신규 확인 사실)"를 반드시 명시하고, 신규 사실을 특정할 수 없으면 직전 밴드 유지. 상세 규칙: ai_eval_prompt.txt.**
