...
- 각 스크립트 역할(참고):
  ...
  → auto_buy.py(주문을 직접 내지 않음. BUY 종목별 목표수량+sweep 기준가만 계산해 logs/{날짜}_autobuy_plan.json 에 기록).
    ※ auto_buy 집행 게이트 = 실시간 total_score×100 ≥ 50점(SCORE_MIN=50). 비중 구간제: 50~59=5% · 60~69=10% · 70~79=15% · 80~89=20% · 90~99=25% · 100=30%. 이 50점 컷은 매수 집행 기준으로, 평가 대상 선정의 60점+ 기준과 별개(집행 게이트는 50점 유지, 평가 대상만 60점+로 상향).
